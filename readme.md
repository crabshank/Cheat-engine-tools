...
* **attach(t, c --[[Optional]] , p --[[Optional]] , bh --[[Optional]] , fw --[[Optional]] )** -> Attach logging breakpoint to address *t* (Use '0x…' for addresses in hexadecimal), or, a table of addresses *t*. *c* is a string or table of strings specifying what to log (could be a register or e.g. register*y+x or, XMM0-15 or FP0-7 (uppercase and case-senstive), depending on whether you're using x64 or x86. Use "0x…", again, for hex offsets e.g. "RAX+0xC". Also, the float registers are interpreted as byte tables, so using them with argument *p* is undefined behaviour). If *p* is set to **true**, then the string(s) in *c* is/are interpreted as a memory address(es)
