...
* **lite( a, c, z --[[Optional]], s --[[Optional]] )** -> Like "traceCount.attach(…)" but only the instructions are logged, so it's quicker. **a**, **c** and **z** are the same as in "traceCount.attach(…)", **s**, if true, will "step over", otherwise it will step into.
